...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len) {
	...
	if (checkTable(receive.id)){
		fillTable(receive.id); // record packet id
		if (receive.type == 1){
...
void sendCurrent (){
	...
	send.id = rand() % 1+50000; // random id 
	send.type = 4; // data
	float data[2] = {a, b};
	memcpy(send.sender, mac, sizeof(mac)); // gateway ip for rts
	memcpy(send.receiver, receiver, sizeof(receiver)); // packet receiver mac
...
void fillTable(char senderMac[18], string packetID){
	for(int x=0; x<20; x++) {
		if(x < 19){
			if(knownPacket[x].currentSender == 0){
